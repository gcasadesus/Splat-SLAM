...
class DepthVideo:
    ...
    def __init__(self, cfg, printer):
        ...
        self.valid_depth_mask_small = torch.zeros(
            buffer,
            ht // self.down_scale,
            wd // self.down_scale,
            device=self.device,
    ...
    def reproject(self, ii, jj):
        ...
        coords, valid_mask = pops.projective_transform(Gs, self.disps[None], self.intrinsics[None], ii, jj)
    ...
    def distance(self, ii=None, jj=None, beta=0.3, bidirectional=True):
        ...
        if bidirectional:
            ...
            d1 = droid_backends.frame_distance(poses, self.disps, self.intrinsics[0], ii, jj, beta)

            d2 = droid_backends.frame_distance(poses, self.disps, self.intrinsics[0], jj, ii, beta)

            d = 0.5 * (d1 + d2)

        else:
            d = droid_backends.frame_distance(self.poses, self.disps, self.intrinsics[0], ii, jj, beta)
    ...
    def dspo(
        ...
        with self.get_lock():
            ...
            if opt_type == "pose_depth":
                target = (
                    target.view(-1, self.ht // self.down_scale, self.wd // self.down_scale, 2)
                    .permute(0, 3, 1, 2)
                    .contiguous()
                )
                weight = (
                    weight.view(-1, self.ht // self.down_scale, self.wd // self.down_scale, 2)
                    .permute(0, 3, 1, 2)
                    .contiguous()
                )
                droid_backends.ba(
                ...
                if self.mono_thres:
                    ...
                    invalid_mono_mask = (
                        (error_t / avg_disps > self.mono_thres)
                        | (error_t.isnan())
                        | (scale_t < 0)
                ...
                for _ in range(itrs):
                    if self.counter.value > ignore_frames and ii_t.shape[0] > 0:
                        poses, disps, wqs = thirdparty.glorie_slam.geom.ba.BA_with_scale_shift(
                            ...
                            scales[None],
                            shifts[None],
                            self.valid_depth_mask_small[None],
                            ignore_frames,
                            lm,
                            ep,
                            alpha=0.01,
    ...
    def update_valid_depth_mask(self, up=True):
        ...
        count = droid_backends.depth_filter(
            self.poses,
            self.disps_up if up else self.disps,
            intrinsic,
            dirty_index,
            thresh,
        )
    ...
    def eval_depth_l1(self, npz_path, stream, global_scale=None):
        ...
        return (
            np.asarray(depth_l1_list).mean(),
            np.asarray(depth_l1_list_max_4m).mean(),
            np.asarray(mask_list).mean(),
        )
